...
# Chapter 2: Maclaurin Pipeline Specification — v1.1
# WPMS シンセサイザー — 第1層仕様書
# 第2章：マクローリンパイプライン仕様 — v1.1
...
## Changelog v1.0 → v1.1 (2026-09-23) / 改訂履歴

Two errata, both raised in correspondence with the PTSG-WPMS-Formation amanuensis after Deliverable 1 (L2 Formation register map v0.1/v0.2) was reviewed. Changed passages are marked **[v1.1]**. Nothing else in the chapter is altered; decisions C2-D1…C2-D15 stand, with C2-D13's amplitude format amended as described in erratum 2.

**Erratum 1 — § 2.5.2 / § 2.5.3, clock-budget wording.** v1.0 mixed three quantities of different kinds: a per-bin "35-clock budget", a "1,016 cycles per module" figure, and "~17 cycles per sample period". The unified statement is: *per sample period, per module, 2,083 clocks against 2,048 bins; the difference of 35 clocks is the pipeline-overhead margin — not a per-bin budget and not a ratio; the 16-clock pipeline depth is a quantity of a different kind (latency, not throughput).* Credit: PTSG-WPMS-Formation amanuensis, 2026-09-04.

**Erratum 2 — amplitude format Q10.14 → Q1.31.** v1.0 carried the C5G-era 24-bit amplitude (10 integer + 14 fractional bits) into § 2.4.1, § 2.4.2 and § 2.7.2. The 10 integer bits were an artifact of C5G's integer level multiplier; with per-bin amplitude ≤ 1 by convention and output scaling handled by the DIP decimal-point selector, they are unnecessary. The L2 Formation register map carries amplitude in a 32-bit slot; under the Free Precision Floor the amplitude multiplier input becomes **Q1.31 signed (non-negative by convention)**, ratified by the architect on 2026-09-23 (PTSG-WPMS-Formation Decision Register W-R10). The product and accumulator widths in § 2.4.2 / § 2.7.2 are restated accordingly; the justification wording there is tightened at the same time. Chapter 1 § 1.6's "24-bit amplitude" line is to be aligned in a Chapter 1 v1.1.

**Editorial note (non-normative) — vocabulary.** The "sequence-modulation processor" named in v1.0 as the upstream of § 2.6 is realized as two things: the **L1 difference engine** (hardwired, one bin per clock — the entity that actually delivers `phase_in`) driven by the **L2 Formation** (PTSG-WPMS-Formation, one Stay = one packet). The PTSG side has retired the labels "Upper/Lower"; this chapter now uses L1 / L2 / L4. Wording in § 2.1 and § 2.6 is adjusted; no decision changes.

**要旨:** 正誤 2 件。(1) § 2.5 のクロック予算の表現を統一——「サンプル周期あたりモジュールあたり 2,083 クロックに対し 2,048 ビン、差の 35 クロックがパイプラインオーバーヘッドのマージン。16 クロックのパイプライン深度は別種の量」。(2) 振幅フォーマットを Q10.14 から **Q1.31** へ——C5G 由来の整数部 10 ビットは不要であり、Free Precision Floor に従い 32 ビットスロット全幅を小数部に充てる（2026-09-23 裁定、W-R10）。加えて語彙を L1 / L2 / L4 に揃える（非規範）。変更箇所は **[v1.1]** で示す。
...
- The input contract from the L1 difference engine (driven by the L2 Formation) **[v1.1]** / L1 差分エンジン（L2 Formation が駆動）からの入力契約
...
- The L1 difference engine and the L2 Formation that together produce the phase accumulator value (Chapter 3; the L2 Formation's ISA-visible register map is owned by the PTSG-WPMS-Formation repository) **[v1.1]** / 位相累算器値を生成する L1 差分エンジンと L2 Formation（第 3 章；L2 Formation の ISA 可視レジスタマップは PTSG-WPMS-Formation リポジトリの所有）
...
- The Compact configuration leaves ample margin: the 16-clock depth of Approach B (2 clocks/stage) is well inside the 35-clock pipeline-overhead margin per sample period per module (§ 2.5.2). **[v1.1]** / Compact 構成は十分な余裕を残す：アプローチ B（2 クロック/段）の 16 クロック深度は、サンプル周期あたりモジュールあたり 35 クロックのパイプラインオーバーヘッドマージン（§ 2.5.2）の内側に十分収まる。
...
- **[v1.1]** The amplitude multiplier (sin(x) × A_k, Chapter 4 territory) operates at full **Q0.40 × Q1.31** precision, producing a **Q1.71** product (73 bits) that is truncated to **Q1.63** (64 bits) at the multiplier output. Compared with v1.0's Q10.54, the same 64-bit width now carries 9 more fractional bits, because the 9 integer bits were never needed. This multiply may consume 2–4 DSP blocks, which is acceptable because it is instantiated **once per module** (a second instance only if L and R carry different amplitudes) — a fixed cost of a few blocks per module, not a per-Horner-stage cost. / 振幅乗算器（sin(x) × A_k、第 4 章領域）は完全な **Q0.40 × Q1.31** 精度で動作し、**Q1.71** の積（73 ビット）を生成、乗算器出力で **Q1.63**（64 ビット）に切り詰める。v1.0 の Q10.54 と比べ、同じ 64 ビット幅で小数部が 9 ビット増える——整数部 9 ビットはそもそも不要だったからである。この乗算は 2〜4 DSP ブロックを消費し得るが、**モジュールあたり 1 個**（L/R が異なる振幅を持つ場合のみ 2 個）のインスタンス化なので、Horner 段あたりのコストではなくモジュールあたり数ブロックの固定費であり、許容される。
- **[v1.1]** The summation tree accumulator receives Q1.63 products. Bit growth is log₂(bins): Compact (4,096 bins) needs at least **Q13.63 (76 bits)**, Standard (10,240 bins) at least **Q15.63 (78 bits)**; implementers may widen further (up to ~108 bits) under the Free Precision Floor. The accumulator is implemented as a DSP-block-internal cascaded adder chain (with fabric extension above the DSP block's native accumulator width), consuming no additional DSP multiplier resources. Topology is Chapter 4's. / 総和ツリー累算器は Q1.63 の積を受ける。ビット成長は log₂(ビン数)：Compact（4,096 ビン）は少なくとも **Q13.63（76 ビット）**、Standard（10,240 ビン）は少なくとも **Q15.63（78 ビット）**；実装者は Free Precision Floor の下でさらに（〜108 ビットまで）広げてよい。累算器は DSP ブロック内部の cascaded 加算器チェーン（DSP ブロックのネイティブ累算幅を超える部分はファブリックで拡張）として実装され、追加の DSP 乗算器リソースを消費しない。トポロジは第 4 章の所有。
...
**[v1.1] Clock budget, stated once.** Per sample period, per module: **2,083 clocks** (100 MHz × 20.833 µs) against **2,048 bins** at one bin per clock. The difference, **35 clocks, is the pipeline-overhead margin** of the sample period — it is not a per-bin budget and not a ratio. The **16-clock pipeline depth** above is a quantity of a different kind: it is latency (the delay from a bin's entry to its sin(x) output), not throughput. The two meet only in one derived inequality: the last bin of a sample period enters at clock 2,047 and its result emerges 16 clocks later, so the accumulator flush that closes the period must fit in the remaining 35 − 16 = **19 clocks**. That is comfortable; it is also the figure a Standard or Extended configuration must re-check.

**[v1.1] クロック予算——一度だけ述べる。** サンプル周期あたりモジュールあたり **2,083 クロック**（100 MHz × 20.833 µs）に対し、1 ビン/クロックで **2,048 ビン**。差の **35 クロックはサンプル周期のパイプラインオーバーヘッドマージン**であり、ビンあたりの予算でも比でもない。上の **16 クロックのパイプライン深度**は別種の量——レイテンシ（ビン投入から sin(x) 出力までの遅延）であってスループットではない。両者が出会うのは一つの導出不等式のみ：サンプル周期最後のビンはクロック 2,047 で投入され、その結果は 16 クロック後に現れるので、周期を閉じる累算器フラッシュは残り 35 − 16 = **19 クロック**に収まらねばならない。これは十分な余裕であり、同時に Standard・Extended 構成が再確認すべき数字でもある。
...
**[v1.1]** The pipeline is fully pipelined: one new bin enters each clock cycle, and one sin(x) value emerges each clock cycle (after the 16-clock fill latency at startup). Throughput is therefore exactly one bin per clock; the 35-clock margin of § 2.5.2 is the only slack in the sample period, and it is spent on pipeline drain and the accumulator flush, not on per-bin timing. The L1 difference engine, clocked by the L2 Formation's Stay counter (one Stay = one packet, Stay counter = k), delivers `phase_in` at that same one-per-clock rate with no cycle-accurate lock beyond the shared clock. *(v1.0's "~17 cycles per sample period" and "~1.7 % excess" are withdrawn: they conflated pipeline depth with the period margin.)*

**[v1.1]** パイプラインは完全パイプライン化される：1 クロックサイクルごとに新しいビンが入り、1 クロックサイクルごとに 1 個の sin(x) 値が出る（起動時の 16 クロック充填レイテンシ後）。したがってスループットは厳密に 1 ビン/クロックであり、§ 2.5.2 の 35 クロックマージンがサンプル周期内の唯一の余裕で、それはパイプラインの排出と累算器フラッシュに費やされ、ビンごとのタイミングには費やされない。L2 Formation のステイカウンタ（一 Stay = 一パケット、ステイカウンタ = k）にクロックされる L1 差分エンジンは、共有クロック以外のサイクル正確なロックなしに、同じ 1/クロックのレートで `phase_in` を配送する。*（v1.0 の「サンプル期間あたり約 17 サイクル」「約 1.7% 超過」は撤回する：パイプライン深度と周期マージンを混同していた。）*
...
**Per-module DSP usage** (Compact configuration, 1 Maclaurin core per module): 7 DSP blocks for the Maclaurin core. With 2 modules: **14 DSP blocks total for both Maclaurin cores combined**. This leaves 112 − 14 = 98 DSP blocks for the rest of the design (L1 difference engine, L2 Formation, amplitude multipliers, summation accumulators, HDMI core, optional camera block). **[v1.1]**
...
### 2.6.1 Interface from the L1 difference engine / L1 差分エンジンからのインターフェース **[v1.1]**

The Maclaurin pipeline accepts one input per clock from the L1 difference engine (Chapter 3), which is driven by the L2 Formation (PTSG-WPMS-Formation):

マクローリンパイプラインは数列変調プロセッサ（第 3 章）から 1 クロックあたり 1 入力を受け入れる：

| Signal | Width | Format | Direction |
|---|---|---|---|
| `phase_in` | 32 | Q0.32 unsigned | L1 difference engine → Maclaurin |
| `phase_valid` | 1 | active-high | L1 difference engine → Maclaurin |
| `bin_index` | 11 | unsigned (0...2047 within module) | L1 difference engine → Maclaurin (passed-through, used by amplitude stage) |
| `pipeline_ready` | 1 | active-high | Maclaurin → L1 difference engine |

The `pipeline_ready` signal is asserted whenever the Maclaurin pipeline can accept new input. In normal operation (no stalls), `pipeline_ready` is held high continuously. The Maclaurin core does not implement back-pressure; it is the L1 difference engine's responsibility to deliver one valid `phase_in` per clock.
...
- **[v1.1]** The amplitude multiplier is instantiated once per module (twice only if L and R need separate amplitudes), unlike the Horner stages, of which there are five per core. Its input is now **Q1.31** (erratum 2), so the product is Q1.71, truncated to Q1.63 at the output. Letting this one multiplier cost 2–4 DSP blocks instead of 1 is a fixed few-block cost per module, not a per-stage cost. / 振幅乗算器はモジュールあたり 1 個（L/R が別振幅を要する場合のみ 2 個）のインスタンス化であり、コアあたり 5 個ある Horner 段とは異なる。入力は **Q1.31**（正誤 2）となり、積は Q1.71、出力で Q1.63 に切り詰める。この 1 個の乗算器に 1 ではなく 2〜4 DSP ブロックを許すのは、段あたりのコストではなくモジュールあたり数ブロックの固定費である。
...
| C2-D9 | Pipeline depth: 9 logical stages, 16 clocks (Approach B); 14-clock reserve from 30-clock budget. Period budget stated as 2,083 clocks vs 2,048 bins, 35-clock margin (v1.1, erratum 1) | Fixed (wording v1.1) |
...
| C2-D13 | Output contract: sin_out (Q0.40, 41 bits), sin_valid, bin_index_out; downstream amplitude input **Q1.31** (v1.1, erratum 2; was Q10.14) | Fixed (amended v1.1) |
...
This chapter is released into the public domain under CC0 1.0 Universal. Chapter 3 (L1 difference engine and its binding to the L2 Formation; see the PTSG-WPMS-Formation repository for the L2 register map) follows. **[v1.1]**
